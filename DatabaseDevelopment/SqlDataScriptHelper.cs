        ...
        public static string NewMergeScriptFromDataTable(string tableSchema, string tableName, DataTable dataTable, bool deleteWhenNotMatched, bool printChanges, string environmentName, List<string> primaryKeyColumnNames = null)
        {
            ...
            for (int i = 0; i < dataTable.Columns.Count; i++)
            {
                ...
                if (!primaryKeyNameList.Contains(currentColumnName))
                {
                    ...
                    sourceUpdateValuesString += $"  [{currentColumnName}] = Source.[{currentColumnName}]";
                    if (i < (dataTable.Columns.Count - 1))
                    {
            ...
            string libraryVersion = FileVersionInfo.GetVersionInfo(System.Reflection.Assembly.GetExecutingAssembly().Location).FileVersion;
            ...
            mergeScript += $"USING (VALUES" + Environment.NewLine;
            mergeScript += $"{rowValuesString}" + Environment.NewLine;
