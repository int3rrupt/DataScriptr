...
namespace DataScriptr.Library.Models.Scripting
{
    public class DataScriptCollection
    {
        ...
        private Dictionary<string, Dictionary<DatabaseEnvironment, DataScript>> EnumerateDataScripts(string databaseProjectDirectoryPath, DataScriptType dataScriptType)
        {
            ...
            {
                FileInfo[] dataScriptFiles = dataScriptsDirectoryInfo.GetFiles("*.data.sql", SearchOption.AllDirectories);
                ...
                foreach (FileInfo staticDataFileInfo in dataScriptFiles)
                {
                    try
                    {
                        ...
                        {
                            Name = Path.GetFileNameWithoutExtension(staticDataFileInfo.Name).Replace(".data", string.Empty),
                            Type = dataScriptType,
