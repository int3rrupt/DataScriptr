...
namespace DatabaseDevelopment
{
    public class DataExporter
    {
        ...
        public DataExporter(string serverName, string databaseName, string configServerName, string configDatabaseName, DataReader dataReader)
        {
            ServerName = serverName;
            DatabaseName = databaseName;
            this._dataReader = dataReader;
            PrimaryKeyConstraints = this._dataReader.GetPrimaryKeyConstraintsFromDatabase();
            ForeignKeyConstraints = this._dataReader.GetForeignKeyConstraintsFromDatabase();
            SoftConstraints = this._dataReader.GetSoftConstraintsFromDatabase(configServerName, configDatabaseName);
            IgnoredDependencyLists = this._dataReader.GetIgnoredDependenciesFromDatabase(configServerName, configDatabaseName);
        ...
        public Dictionary<string, ExtractedTable> ExportDataFromDatabase(string query, bool includeSoftDependencies, bool excludeUnwantedDependencies)
        {
            ...
            foreach (SelectStatement selectStatement in selectStatementsFromQuery)
            {
                ...
                DataSet result = this._dataReader.ExecuteQuery(selectStatement.SelectText);
                // Get primary key constraints for current table
                List<string> primaryKeyColumnNames = this._dataReader.GetListOfPrimaryKeyConstraints(selectStatement.PrimaryTableName, PrimaryKeyConstraints).Select(row => row["ColumnName"].ToString()).ToList();
                // Get primary keys from query results
                Dictionary<string, List<string>> primaryKeysAndValues = this._dataReader.GetPrimaryKeyValues(selectStatement.PrimaryTableName, result.Tables[0], PrimaryKeyConstraints);
                ...
                ExtractedTable extractedTable = new ExtractedTable
                {
                    ...
                    DataTable = this._dataReader.ExecuteQueryUsingForXml(ServerName, DatabaseName, selectUsingPrimaryKeys).Tables[0]
                };
        ...
        public DataTable RetrieveTable(string serverName, string databaseName, TableName parentTableName, Dictionary<string, List<string>> keysAndValues, DataTable primaryKeyConstraints)
        {// TODO move this logic into RetrieveTableWhereKeysInValues
            DataTable dataTable = new DataTable();
            if (keysAndValues.Count > 0)
            { 
                // Determine primary key constraints
                List<string> primaryKeyColumnNames = this._dataReader.GetListOfPrimaryKeyConstraints(parentTableName.Schema, parentTableName.Name, primaryKeyConstraints).Select(row => row["ColumnName"].ToString()).ToList();
                dataTable = this._dataReader.RetrieveTableWhereKeysInValues(serverName, databaseName, parentTableName, keysAndValues, primaryKeyColumnNames);
            }
            else
            {
                //Write-Host "Table $parentTableNameDelimited not retrieved. $childTableNameDelimited.[$(foreignKeyConstraint.ConstraintColumnName)] values are all DBNull"; todo
